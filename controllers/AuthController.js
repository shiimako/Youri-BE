...
const authController = {
  ...
  register: async (req, res) => {
    try {
      ...
      if (!password) {
        ...
        errors.password = [];
        if (password.length < 6) errors.password.push("Password minimal 6 karakter");
        if (!/[A-Z]/.test(password)) errors.password.push("Password harus memiliki minimal 1 huruf besar");
        if (!/[0-9]/.test(password)) errors.password.push("Password harus memiliki minimal 1 angka");
  ...
  login: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Terjadi kesalahan internal server saat login" });
    }
  },
  ...
  requestPasswordReset: async (req, res) => {
    try {
      ...
      const mailOptions = {
        from: `"Youri Support" <${process.env.EMAIL_ADMIN}>`,
        to: user.email,
      ...
      return res.status(200).json({ message: "Kode OTP pemulihan telah dikirim ke email" });

    } catch (error) {
      console.error("[Request Reset Password Error]:", error);
      res.status(500).json({ message: "Gagal memproses permintaan pemulihan sandi" });
    }
  },
  ...
  verifyOTP: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Terjadi kesalahan internal saat memverifikasi OTP" });
    }
  },
  ...
  resetPassword: async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: "Kata sandi berhasil diperbarui, silakan login kembali" });
